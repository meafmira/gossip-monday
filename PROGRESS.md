...
- 2026-06-17: Installed the `impeccable` design skill (vendored from github.com/pbakaus/impeccable into `.agents/skills/impeccable`; the official `impeccable.style` installer host is NOT on the env egress allowlist, so npx install 403s — fetch the bundle from GitHub instead). Ran its audit/critique, then a `polish` pass over `src/styles/global.css`, the section components, and `src/scripts/app.ts`.
  ...
  - **Fonts (swapped 2026-06-17, second pass):** replaced the two reflex-reject faces. Display `Playfair Display → Oswald` (condensed gothic, tabloid-masthead energy); body `Inter → Onest` (contemporary grotesque). Both are OFF impeccable's reject list + the detector's overused list, and **both serve the full Cyrillic block** (verified via the Google Fonts CSS2 `/* cyrillic */` subset, `unicode-range: U+0400-045F …`) — mandatory since the site is Russian. Font URL in `Base.astro`: `family=Onest:wght@400..900&family=Oswald:wght@500..700`. Oswald is condensed, so headline `letter-spacing` was relaxed to ~`-0.01em` and `line-height` loosened to `0.95` so Cyrillic caps (Й/Ё) don't clip; display headings set `font-weight: 700` (Oswald's ceiling). Detector now only reports `single-font` — a false positive (Oswald is applied via CSS class, invisible to the static-HTML scanner). NOT visually verified: no Chromium in sandbox; renders in prod via Google Fonts.
